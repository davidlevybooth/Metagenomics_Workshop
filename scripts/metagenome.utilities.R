...
tree_bar_plot <- function(long.df, label, colors = c("cadetblue", "gold"), fontsize = 6) {
  p <- ggplot(long.df, 
    ...
    theme(legend.position = "none", #Don't need a legend b/c of y-label
          ...
          axis.title.x = element_text(size = fontsize), #change x-axis font size
          axis.text.x = element_text(size = fontsize), #change x-axis font size
...
relative_abundance <- function(count_table) {
  ...
  if(nums == ncol(count_table)) {
    count_table.num = count_table
