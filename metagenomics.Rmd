...
At the beginning of every R script, you should have a dedicated space for loading R packages. R packages allow any R user to code reproducible functions and share them with the R community. Here, we load the necessary packages which must already be installed (see setup instructions for details).
...
> Question: How can you access it? (tip: look at help page of function ape::read.tree)
...
```{r eval=FALSE}
data.frame[1:4, ] # I got an error here: "object of type 'closure' is not subsettable"
```
...
Second, we have to learn to use the findMRCA() function in the phytools package. This allows us to find the common ancestor nodes for a list of tree tips. 
...
The "R" way to run this would be using an apply function, as R is not engineered to effectively use "FOR" loops. But they work in a pinch:
...
ggtree is sometimes counterintuitive. Case in point geom_nodepoint() counts tips as nodes. To account for this we have to build a vector of bootstrap values that is offset by the number of tips for correct bootstrap placement in tree. 
...
> ADVANCED: How would you go about highlighting only your genome of interest? (tips ?)
...
function.plot
# Git an error here: 
#Error: At least one layer must contain all faceting variables: `Function`.
#* Plot is missing `Function`
#* Layer 1 is missing `Function`#
