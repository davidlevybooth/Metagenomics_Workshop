...
Please come to the workshop with your laptop setup with the required software and data files as described
...
At the beginning of every R script, you should have a dedicated space for loading R packages. R packages
allow any R user to code reproducible functions and share them with the R community. Here, we load the necessary packages which must already be installed (see setup instructions for details).
...
### Downloading data
The following command downloads the data from our [GitHub](https://github.com/davidlevybooth/Metagenomics_Workshop) and since you're working in a Project, saves it in the Project directory under the 'data' directory on your computer.
...
```{r eval=FALSE}
download.file(
  "https://raw.githubusercontent.com/EDUCE-UBC/workshop_data/master/metagenome.utilities.R",
...
Note that GTDB-Tk will seperately analyize and place archaeal and bacterial genomes using 122 or 120 single copy genes (SCGs). We then used the bind.tree() function to combine both trees at their default roots. This combined tree is what is provided for you. 
...
#Load data
GTDB.taxonomy <- read.csv(GTDB.taxonomy.path, sep="\t")
...
> Question: What other data are associated with tree objects?
...
```{r}
tips.keep <- sample_gtdb_tips(tips, 1000); length(tips.keep)
```
...
> Question: How else could we have accomplished this? 
...
First, we have to use the taxonomy table to select only Firmicutes_A from the GTDB. 
...
To demonstrate theis we will subset the GTDB taxonomy table to only our clade of interest. We then only take the GenomeIDs for these subset genomes. We must also ensure that the resulting list is a character vector because a factor will do screwy things to the findMRCA() function. 
...
Firmicutes_A_tips <- as.character(GTDB.taxonomy[which(GTDB.taxonomy[,"Phylum"] == Clade),]$GenomeID)


head(Firmicutes_A_tips)
```

Second, we have to learn to use the findMRCA() function in the phytools package. This allows us to find the common ancesor nodes for a list of tree tips. 
...
Next, we will create the collapse_nodes() function to apply findMRCA() over the whole tree to collect nodes for all phyla. 
...
> ADVANCED: Plot highlights for other clades. How do you highlight more than one clade? 
...
#Is there a different/better way to achieve this functionality in R? 
...
ggtree is sometimes counterintuative. Case in point geom_nodepoint() counts tips as nodes. To account for this we have to build a vector of bootstrap values that is offset by the number of tips for correct bootstrap placement in tree. 
...
> Excersise: Using the above functions, find and highlight the phylum of your genome in a tree with bootstrap values and a scale
...
Add arbritary color ranges with the predefined some_color() function. 
...
> Question: What's the deal with the raw node values in the GTDB tree? How do we even find this information?  
...
> ADVANCED: How would you go about highlighting only your genome of interest?

# Section 3. MAG statstic plots

We can use checkM data to map genome binning metrics to MAG tree. CheckM uses single copy genes (SCGs) to determine the levels of completness and contamination ("redundancy") in a genome bin. We also use bbmap to calculate coverage. 

```{r}
MAG.checkM.path <- "C:/Users/user/Desktop/Workshop/Data/checkm.qa.tsv"
...
Create seperate 'completeness' data.frame (still in "long" format). 
...
> Answer: GREP stands for Global Regular Expression Print. We can match strings (characters), including with a wide variety of "wildcards" to subset a data.frame() by creating a numeric index vector for matching entires. 

> grepl(), (grep list) in contrast, produces a boolean vector. This is really useful for certain functions like ifelse() that apply different outcomes based on booleans. 
...
This is how we would create the completness plot with ggplot2. 
...
* _factor()_ re-orders hierarchy of data.frame row entires without changing the data itself
...
counts.long$Label <- paste(counts.long$Genome, " (", counts.long$Classification, ")", sep = "")
...
These steps have already been run for you and the results tables for all genomes concatonated. This organization works well for ggplot2, in which data is primarily supplied to in melted (long) format. 
...
KO.pathways <- read.table("C:/Users/user/Desktop/Workshop/KO.pathways.tsv", sep = "\t", header = TRUE)
...
KO.mag <- read.delim("C:/Users/user/Desktop/Workshop/KO.genomes.tsv", sep = "\t", header = TRUE)
...
Metatranscriptome analysis can be difficult to analyze effectively. In a traditional transcriptome study, we target cells generally in the same growth phase to obtain our desired signal. In an environmental sample, or even in an incubation, memebers of the microbiome may be in wildly different growth phases. 
...
To understand the broad changes in the Liard hot spring transcriptome during incubations with different lignin substrates, we will first analyize reads that have been mapped with Meiji to our refined genome bins from above, but also less-complete genomic and transcriptomic bins. 
...
Next we use the sample heatmap plot as above to see if we can find bins that are particularly active in our various lignin compounds: 
...
Next, we use the genomic plotting fuctions in _circlize_ to plot the expression coverage and genome open reading frames (ORFs). To learn much more about plotting with circlize, please refer to the [circlize ebook](https://jokergoo.github.io/circlize_book/book/). 
...
#We begin bit intializing the plotting parameters
